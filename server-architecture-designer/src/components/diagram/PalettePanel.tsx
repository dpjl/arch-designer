...
const PaletteItem: React.FC<PaletteItemProps> = ({ entry, onDragStart }) => (
  <div
    ...
    className="group relative aspect-square rounded-lg border bg-white/80 dark:bg-slate-700/60 hover:bg-white dark:hover:bg-slate-600 hover:shadow-md cursor-grab active:cursor-grabbing transition-all duration-200 flex items-center justify-center p-2"
  >
  ...
  <Card className="rounded-2xl bg-white/80 dark:bg-slate-800/70 backdrop-blur border-slate-200 dark:border-slate-700">
      ...
      <CardContent className="space-y-5">
        {['container','service','security','generic'].map(cat => (
          <div key={cat}>
            <h4 className="text-xs font-medium text-slate-600 dark:text-slate-300 mb-2 uppercase tracking-wide">{cat === 'container' ? 'Containers' : cat.charAt(0).toUpperCase()+cat.slice(1)}</h4>
        ...
        <div>
          <h4 className="text-xs font-medium text-slate-600 dark:text-slate-300 mb-2 uppercase tracking-wide">Networks</h4>
          <div className="grid grid-cols-6 gap-2">
