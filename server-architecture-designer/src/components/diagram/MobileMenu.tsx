...
export interface MobileMenuProps extends Pick<ToolbarProps,
  'mode' | 'setMode' | 'onSave' | 'onLoad' | 'onExportPng' | 'onExportPdf' | 'onExportJson' | 'onImportJson' | 'onClear' | 'onUndo' | 'onRedo' | 'canUndo' | 'canRedo' | 'snapEnabled' | 'setSnapEnabled' | 'onSnapAll'
> {
...
export const MobileMenu = memo(({ mode, setMode, onSave, onLoad, onExportPng, onExportPdf, onExportJson, onImportJson, onClear, undo, redo, onUndo, onRedo, canUndo, canRedo, snapEnabled, setSnapEnabled, onSnapAll }: MobileMenuProps) => {
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 p-3 rounded-xl bg-white shadow-lg border space-y-2 z-50">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            ...
            <button onClick={()=>onExportPdf('portrait')} className="px-2 py-1 rounded bg-slate-100 hover:bg-slate-200">PDF</button>
