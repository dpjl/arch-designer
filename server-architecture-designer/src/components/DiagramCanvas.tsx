...
const ComponentNode = memo(({ id, data, selected, isConnectable, xPos, yPos }: any) => {
  ...
  const {
    label = 'Component', icon, color = '#94a3b8', features = {}, bgColor = '#ffffff', bgOpacity = 1,
    isContainer = false, width = 520, height = 320, locked = false
  } = data || {};
  ...
  const startX = e.clientX, startY = e.clientY; const startW = width, startH = height;
    ...
    const up = () => { window.removeEventListener('mousemove', move); window.removeEventListener('mouseup', up); document.body.classList.remove('resizing-container');
      const setNodesFn = (window as any).__setDiagramNodes; if (typeof setNodesFn === 'function') setNodesFn((nds: any[]) => nds.map(n => n.id === id ? { ...n, draggable: true } : n)); };
    window.addEventListener('mousemove', move); window.addEventListener('mouseup', up);
  };

  if (isContainer) {
    return (
  <div className="relative" style={{ width, height, ['--fwtexH' as any]: features?.firewall ? `url('${getBrickTexture().urlH}')` : undefined, ['--fwtexV' as any]: features?.firewall ? `url('${getBrickTexture().urlV}')` : undefined, ['--fwtexSize' as any]: features?.firewall ? `${getBrickTexture().size}px ${getBrickTexture().size}px` : undefined, ['--fwtexOffX' as any]: `${getBrickTexture().offX}px`, ['--fwtexOffY' as any]: `${getBrickTexture().offY}px`, ['--fwShiftTopY' as any]: `${getBrickTexture().shiftTopY}px`, ['--fwShiftSideX' as any]: `${getBrickTexture().shiftSideX}px`, ['--ringGapInner' as any]: '8px', ['--ringThickness' as any]: '14px' }}>
        {features?.firewall && (
          <>
  ...
  return (
  <div className="relative inline-block" style={{ ['--fwtexH' as any]: features?.firewall ? `url('${getBrickTexture().urlH}')` : undefined, ['--fwtexV' as any]: features?.firewall ? `url('${getBrickTexture().urlV}')` : undefined, ['--fwtexSize' as any]: features?.firewall ? `${getBrickTexture().size}px ${getBrickTexture().size}px` : undefined, ['--fwtexOffX' as any]: `${getBrickTexture().offX}px`, ['--fwtexOffY' as any]: `${getBrickTexture().offY}px`, ['--fwShiftTopY' as any]: `${getBrickTexture().shiftTopY}px`, ['--fwShiftSideX' as any]: `${getBrickTexture().shiftSideX}px`, ['--ringGapInner' as any]: '5px', ['--ringThickness' as any]: '12px' }}>
      {features?.firewall && (
        <>
          <div className="firewall-ring rounded-2xl">
            <div className="fw-top" />
  ...
  <div className="group rounded-2xl shadow-lg px-2 py-1 w-[240px] hover:shadow-xl transition overflow-visible border relative" style={{ borderColor, background: bg }}>
        {Array.isArray(data?.networkColors) && data.networkColors.length > 0 && (
          <div className="absolute left-0 right-0 top-0 h-1.5 flex overflow-hidden rounded-t-2xl">
            {data.networkColors.slice(0,8).map((c:string, i:number) => (
  ...
  {/* Bigger, more forgiving handles; visible only when selected */}
  ...
  <Handle type="source" position={Position.Right} className={`handle-lg !bg-gray-500/80 transition-opacity ${selected ? 'opacity-100' : 'opacity-0'}`} isConnectable={isConnectable} />
        ...
        {Array.isArray(data?.instances) && data.instances.length > 0 && (
          ...
          {shown.map((ins: any, i: number) => (
                    <div
                      key={i}
                      className="pointer-events-none px-1.5 py-0.5 rounded-t-lg border border-b-0 text-[10px] shadow-sm max-w-[100px] flex items-center gap-0.5"
                      style={{ borderColor, background: ins?.bgColor ? ins.bgColor : 'linear-gradient(to bottom, #ffffff, #f8fafc)', color: ins?.fgColor ? ins.fgColor : (ins?.bgColor ? autoTextColor(ins.bgColor) : '#111827') }}
                    >
                      <span className="font-mono truncate" style={{ maxWidth: '66px' }}>{short(ins?.id)}</span>
                      {ins?.auth ? (
                        <span className="inline-flex flex-col leading-[0.8] text-[9px]">
                          <span>🔑</span>
                          {ins.auth === 'auth2' && <span>🔑</span>}
                        </span>
                      ) : null}
                    </div>
                  ))}
                  {more > 0 && (
                    <div
                      className="pointer-events-none px-2 py-0.5 rounded-t-lg border border-b-0 text-slate-600 text-[10px] shadow-sm bg-slate-50"
                      style={{ borderColor }}
                    >
                      +{more}
                    </div>
...
function snapDoorToNearestSide(parent: any, localPos: {x:number;y:number}, doorW: number, doorH: number, offsets?: { top?: number; bottom?: number; left?: number; right?: number }) {
  ...
  const dx = Math.min(localPos.x, w - localPos.x);
  const dy = Math.min(localPos.y, h - localPos.y);
  let side: 'top'|'bottom'|'left'|'right' = 'top';
  // choose nearest boundary
  if (dy <= dx) {
    side = (localPos.y < h/2) ? 'top' : 'bottom';
  } else {
    side = (localPos.x < w/2) ? 'left' : 'right';
  }
  return placeDoorOnSide(parent, side, localPos, doorW, doorH, offsets);
}
...
function placeDoorOnSide(parent:any, side:'top'|'bottom'|'left'|'right', localPos:{x:number;y:number}, doorW:number, doorH:number, offsets?: { top?: number; bottom?: number; left?: number; right?: number }){
  const w = parent?.data?.width || 520; const h = parent?.data?.height || 320;
  let x = localPos.x, y = localPos.y;
  const ftTop = offsets?.top ?? 0;
  const ftBottom = offsets?.bottom ?? 0;
  const ftLeft = offsets?.left ?? 0;
  const ftRight = offsets?.right ?? 0;
  switch(side){
    case 'top': {
      // center on ring band: shift by ring gap + half ring thickness, then fine-tune
      y = - (CONTAINER_RING_GAP + CONTAINER_RING_THICKNESS/2) - doorH/2 + ftTop;
  x = clamp(x, -doorW/2, w - doorW/2);
      break;
    }
    case 'bottom': {
      y = h - (CONTAINER_RING_GAP + CONTAINER_RING_THICKNESS/2) - doorH/2 - ftBottom;
      x = clamp(x, -doorW/2, w - doorW/2);
      break;
    }
    case 'left': {
      x = - (CONTAINER_RING_GAP + CONTAINER_RING_THICKNESS/2) - doorW/2 + ftLeft;
      y = clamp(y, -doorH/2, h - doorH/2);
      break;
    }
    case 'right': {
      x = w - (CONTAINER_RING_GAP + CONTAINER_RING_THICKNESS/2) - doorW/2 - ftRight;
      y = clamp(y, -doorH/2, h - doorH/2);
      break;
    }
  }
...
const DoorNode = memo(({ data, selected }: any) => {
  ...
  const barrierL = Math.max(Math.round(doorW * 0.9), 64); // slightly longer
  const barrierH = Math.max(8, Math.round(doorH * 0.36)); // ~2x thicker
  ...
  return (
    <div className="relative select-none" style={{ width: doorW, height: doorH }}>
      {/* Bridge meant to sit on firewall band */}
      <div className="absolute inset-0" style={{ transform: `rotate(${rotate}deg)`, transformOrigin: 'center' }}>
        {/* barrier anchored at the left edge of the door, above body */}
        ...
        {/* door body with visible side posts */}
    <div className="relative h-full w-full flex items-stretch">
          ...
          <div className="flex-1 bg-white text-black flex items-center justify-center px-3 relative overflow-visible">
            ...
            {/* central handles to allow connecting to doors; visible only when selected */}
